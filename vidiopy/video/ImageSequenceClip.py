...
class ImageSequenceClip(VideoClip):
    ...
    def __init__(self,
                 sequence: str | Path | tuple[Image.Image, ...] | tuple[np.ndarray, ...] | tuple[str | Path, ...],
                 fps: int | float | None = None,
                 duration: int | float | None = None,
                 audio=None):
        super().__init__()

        self.clip: tuple[Image.Image, ...] = self._import_image_sequence(
            sequence)
        if fps is not None and duration is not None:
            ...
            if duration is None:
                raise ValueError(
                    "You must specify either fps or duration.")
    ...
    def _import_image_sequence(self,
                               sequence: str | Path | tuple[str |
                                                            Path] | tuple[Image.Image] | tuple[np.ndarray]
                               ) -> tuple[Image.Image, ...]:
        if isinstance(sequence, tuple):
            if isinstance(sequence[0], Image.Image):
                ...
                return tuple(map(Image.fromarray, sequence))
                    ...
                    (hasattr(sequence[0], 'read') and
                     callable(getattr(sequence[0], 'read'))):
            ...
            files = [os.path.join(sequence, file) for file in os.listdir(sequence)
                     if os.path.isfile(os.path.join(sequence, file)) and
                     os.path.splitext(file)[1].lower() in set(Image.registered_extensions().keys())]
            files.sort()
    ...
    def make_frame_array(self, t) -> np.ndarray:
        time_per_frame = (
            self.duration if self.duration else self.end) / len(self.clip)
    ...
    @override
    def fl_frame_transform(self, func: Callable[[Image.Image, tuple[Any], dict[str, Any]], Image.Image], *args, **kwargs) -> Self:
